...
//         <h1 style={{ margin: '45px 20px 8px 180px', fontSize: "20px" }}>
...
//             position: 'absolute',
//             top: '77px',
//             left: '420px' // подправь по месту, чтобы не наезжало на "назад"
//         }}
//           >
//             ⟳ Сбросить
//         </button>
...
function App() {
  ...
  useEffect(() => {
    const allSaved = allTasks.filter(task => {
      const saved = localStorage.getItem(getTaskKey(task.id));
      return saved !== null;
    });

    setTotalCorrect(allSaved.length);
  }, [allTasks]);

  // Загрузка прогресса выбранного диапазона
  useEffect(() => {
    if (selectedRange) {
      const tasksInRange = allTasks.filter(
        (t) => t.id >= selectedRange.start && t.id <= selectedRange.end
  ...
  const handleSaveAnswer = (taskId, answer) => {
    const updatedTasks = { ...answeredTasks, [taskId]: answer.toString() };
    const newCount = Object.keys(updatedTasks).length;

    setAnsweredTasks(updatedTasks);
    setCorrectCount(newCount);

    localStorage.setItem(getTaskKey(taskId), answer.toString());
  };
  ...
  const resetCurrentRange = () => {
    ...
    tasksInRange.forEach((task) => {
      localStorage.removeItem(getTaskKey(task.id));
    });
  ...
  // Главная страница — выбор диапазона
  return (
    <div
      style={{
        maxWidth: 600,
